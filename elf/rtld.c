	 ...
	 ElfW(auxv_t) *auxv)
{
     ...
     dependencies in the executable's searchlist for symbol resolution.  */
  {
    RTLD_TIMING_VAR (start);
    rtld_timer_start (&start);
    /*xiaojin load-so -0*/_dl_map_object_deps (main_map, preloads, npreloads, 
...
#define VERNEEDTAG (DT_NUM + DT_THISPROCNUM + DT_VERSIONTAGIDX (DT_VERNEED))
  ...
  if (prelinked)
      ...
      while (i-- > 0)
	{
	  ...
	  if (l != &GL(dl_rtld_map))
	    /*xiaojin relocate -0 开始resolve 程序的符号包括vdso - link map是`dl_sysinfo_map`*/_dl_relocate_object (l, l->l_scope, GLRO(dl_lazy) ? RTLD_LAZY : 0,
				 consider_profiling);
