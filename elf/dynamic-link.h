...
~ xiaojin relocate -2.1 _ELF_DYNAMIC_DO_RELOC (RELA, Rela, map, scope, lazy, skip_ifunc, _ELF_CHECK_REL) 将`.dyn.rela`与`.plt.rela`这两个针对got的重定位信息放入了range[2]
  ...
  do {									      \
	...
	if (!(do_lazy)							      \
      ...
      for (int ranges_index = 0; ranges_index < 2; ++ranges_index)	      \
        /*xiaojin relocate -2.2 */elf_dynamic_do_##reloc ((map), scope,				      \
				ranges[ranges_index].start,		      \
				ranges[ranges_index].size,		      \
				ranges[ranges_index].nrelative,		      \
				ranges[ranges_index].lazy,		      \
				skip_ifunc);				      \
  } while (0)//~ xiaojin elf_dynamic_do_Rel 这个函数
...
/* This can't just be an inline function because GCC is too dumb
   to inline functions containing inlines themselves.  *//*xiaojin relocate -2.0 重定位肯定是这个路径 .dynamic -> rela.* */
